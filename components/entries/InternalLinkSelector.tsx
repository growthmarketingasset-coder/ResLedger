...
const TABLES: Record<string, string> = {
  ...
  tool: 'tools',
  idea: 'ideas',
  ai_strategy: 'ai_strategies',
  workshop_video: 'workshop_videos',
      ...
      {links.length > 0 && (
        <div className="mb-3 flex flex-wrap gap-1.5">
          {links.map((link) => (
            ...
            >
              <span className="shrink-0" style={{ color: 'var(--text-muted)', fontSize: '10px' }}>
                {ENTRY_TYPE_LABELS[link.type] || link.type}
              </span>
              <span className="min-w-0 truncate">{link.title}</span>
              <button onClick={() => removeLink(link.id)} className="ml-0.5 shrink-0 transition-opacity hover:opacity-60">
                <X size={10} />
              </button>
            </span>
          ))}
      ...
      <div className="relative" ref={ref}>
        <div
          ...
          style={{
            ...
            boxShadow: open ? '0 0 0 3px rgba(124,108,242,0.12)' : 'none',
          }}
        >
        ...
        {open && (query || results.length > 0) && (
          <div
            className="absolute left-0 right-0 top-full z-30 mt-1 overflow-hidden rounded-xl animate-scale-in"
